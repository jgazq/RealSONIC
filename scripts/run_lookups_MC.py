...
def computeAStimLookup(pneuron, aref, fref, Aref, fsref, Qref, novertones=0,
    ...
    if fsref.size > 1 or fsref[0] != 1.: #checks if multiple fs are given, otherwise it needs to be 1
        for x in ['a', 'f']:
    ...
    for drive in drives:
        for Qm in refs['Q']:
            queue.append([drive, refs['fs'], Qm]) #queue contains all possible combinations of f,A,fs and Qm
    ...
    dims = np.array([x.size for x in refs.values()]) #dimensions of ['a', 'f', 'A', 'Q', 'AQ1', 'phiQ1', ..., 'AQn', 'phiQn', 'fs']
    ...
    for a in refs['a']:
        ...
        outputs += batch(mpi=mpi, loglevel=loglevel) #effective variables are added to output together with the computation times
    ...
    tcomps = np.array(tcomps).reshape(dims[:-1]) #the tcomps list is rearranged so every input argument can be indexed (# input variables = # dimensions list)
    tcomps = np.moveaxis(np.array([tcomps for i in range(dims[-1])]), 0, -1) #an extra dimsension is added so every value is now a list itself
    tables['tcomp'] = tcomps
...
def main():
    ...
    for pneuron in args['neuron']: 
        ...
        # Check if lookup file already exists -> this becomes useless as time of computations is added to the LUT .pkl file name
