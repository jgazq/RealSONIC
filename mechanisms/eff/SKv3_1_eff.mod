:Comment :
:Reference : :		Characterization of a Shaw-related potassium channel family in rat brain, The EMBO Journal, vol.11, no.7,2473-2486 (1992)

NEURON	{
	SUFFIX SKv3_1
	USEION k READ ek WRITE ik
	RANGE gSKv3_1bar, gSKv3_1, ik 
	RANGE Adrive, Vm, y, Fdrive, A_t : section specific
	RANGE stimon, detailed    : common to all sections (but set as RANGE to be accessible from caller)
}

UNITS	{
	(S) = (siemens)
	(mV) = (millivolt)
	(mA) = (milliamp)
}

PARAMETER	{
	stimon       : Stimulation state
	Fdrive (kHz) : Stimulation frequency
	Adrive (kPa) : Stimulation amplitude
	detailed     : Simulation type
	gSKv3_1bar = 0.00001 (S/cm2) 
}

ASSIGNED	{
	v (nC/cm2)
	Vm (mV)
	ek	(mV)
	ik	(mA/cm2)
	gSKv3_1	(S/cm2)
	mInf
	mTau
	A_t  (kPa)
	y
}

INCLUDE "update.inc"

FUNCTION_TABLE V(A(kPa), Q(nC/cm2)) (mV)
FUNCTION_TABLE alpham_SKv31(A(kPa), Q(nC/cm2)) (/ms)
FUNCTION_TABLE betam_SKv31(A(kPa), Q(nC/cm2)) (/ms)

STATE	{ 
	m
}

BREAKPOINT	{
printf("SKv3_1.mod: \n")
printf("V = %g\n",V(A_t,y))
	update()
	SOLVE states METHOD cnexp
	gSKv3_1 = gSKv3_1bar*m
	ik = gSKv3_1*(Vm-ek)
}

DERIVATIVE states	{
	m' = alpham_SKv31(A_t, y) * (1 - m) - betam_SKv31(A_t, y) * m
}

INITIAL{
printf("SKv3_1.mod: \n")
printf("V = %g\n",V(A_t,y))
	update()
printf("SKv3_1.mod: \n")
printf("V = %g\t",V(A_t,y))
printf("alpha = %g\t" ,alpham_SKv31(A_t, y))
printf("beta = %g\t" ,betam_SKv31(A_t, y))
	m = alpham_SKv31(A_t, y) / (alpham_SKv31(A_t, y) + betam_SKv31(A_t, y))
}

INDEPENDENT {
	t FROM 0 TO 1 WITH 1 (ms)
}