...
FUNCTION_TABLE alpham_Im(A(kPa), Q(nC/cm2), Q1(nC/cm2), phi1(rad)) (/ms)
FUNCTION_TABLE betam_Im(A(kPa), Q(nC/cm2), Q1(nC/cm2), phi1(rad)) (/ms)
...
DERIVATIVE states	{
	m' = alpham_Im(A_t, y, q1, f1) * (1 - m) - betam_Im(A_t, y, q1, f1) * m
}

INITIAL{
	update()
	m = alpham_Im(A_t, y, q1, f1) / (alpham_Im(A_t, y, q1, f1) + betam_Im(A_t, y, q1, f1))
}
